...
$cuvant = $_POST['searchTxt'];
$categorie = $_POST['searchBy'];

if(($cuvant == '') || ($categorie == '')) {
	echo '<br /><br /><br /><p id="albastru">Nu aţi introdus cîmpuri pentru a putea căuta !</p><br />
    	  <p id="albastru2">Apăsaţi <a href="index.php">aici</a> pentru a vă întoarce la pagina principală.</p>';
} 
else 
{
	if($categorie=="toate")
		$cerereSQL = 'SELECT * FROM `carti` WHERE `autor` = "'.$cuvant.'" OR `titlu` = "'.$cuvant.'" OR `editura` = "'.$cuvant.'"' ;
	if($categorie=="titlu")
		$cerereSQL = 'SELECT * FROM `carti` WHERE `titlu` = "'.$cuvant.'"';
	if($categorie=="autor")
		$cerereSQL = 'SELECT * FROM `carti` WHERE `autor` = "'.$cuvant.'"';
	if($categorie=="editura")
		$cerereSQL = 'SELECT * FROM `carti` WHERE `editura` = "'.$cuvant.'"';
	...
	if(mysql_num_rows($rezultat) > 0)		
		...
		<input type="hidden" name="cat" value="'.$rand['categorie'].'" />
		<input type="hidden" name="id_carte" value="'.$rand['id_carte'].'" />
		<input type="submit" name="submit" value="Cumpără" />		
		</form>';
		
		if(isset($_SESSION['e_mail']) && ($_SESSION['e_mail']=="administrator"))
		{
		echo '<a href="profil.php?action=modifica_carti&id='.$rand['id_carte'].'">
		<i id="editsterg">Edit</i></a>';
		echo '<a href="profil.php?action=sterge_carti&id='.$rand['id_carte'].'">
	...
	else echo '<br /><br /><br /><p id="albastru">Nu au fost găsite rezultate !</p><br />
    	  <p id="albastru">Apăsaţi <a href="index.php">aici</a> pentru a vă întoarce la pagina principală.</p>';
}	
?>	
   </div>
   <br class="clearfloat" />
