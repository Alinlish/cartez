...
echo '<br /><strong id="alb_mic">V-aţi abonat cu succes la newsletter !</strong>
      <br /> <a href="index.php"><strong id="alb_mic">Înapoi</strong></a><br />';
	...
	<strong>Căutare carte : &nbsp;</strong>
	 <input name="searchTxt" value="" type="text" id="culoare_box" /> &nbsp;
      <select name="searchBy" id="culoare_box2">
        <option label="Cautare dupa" value="toate">Căutare dupa toate</option>
...
$cerereSQL="SELECT * FROM `carti` WHERE `categorie` = '".$_GET['categorie']."'";
  ...
  echo '<p>
		...
		<div id="prett">Pret: '.$pret.'</div>
		<div id="cumpara">
		<form action="cos_cumparaturi.php?cos=adauga" method="post">
		
		<input type="hidden" name="cat" value="'.$_GET['categorie'].'" />
		<input type="hidden" name="id_carte" value="'.$row['id_carte'].'" />
		<input type="submit" name="submit" value="Cumpara" />		
		</form>';
